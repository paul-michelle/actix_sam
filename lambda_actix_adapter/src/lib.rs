...
impl<S, B> LambdaService<LambdaEvent<ApiGatewayProxyRequest>> for ActixHandler<S, B>
where
    S: actix_service::Service<
            actix_http::Request,
            Response = actix_web::dev::ServiceResponse<B>,
    ...
    <B as actix_web::body::MessageBody>::Error: std::fmt::Debug,
{
    ...
    fn call(&mut self, req: LambdaEvent<ApiGatewayProxyRequest>) -> Self::Future {
        use serde_json::json;

        let event: ApiGatewayProxyRequest = req.payload;
        ...
        // Parse request
        let actix_request = lambda_request_to_actix_request(event);
...
fn lambda_request_to_actix_request(
    event: ApiGatewayProxyRequest,
) -> Result<actix_http::Request, LambdaError> {
    // Construct actix_web request
    // let method = actix_web::http::Method::try_from(event.http_method)?;
    let req =
        actix_web::test::TestRequest::with_uri(&event.path.unwrap()).method(event.http_method);

    // Source IP
    let req = if let Some(source_ip) = event.request_context.identity.source_ip {
        req.peer_addr(std::net::SocketAddr::from_str(&source_ip).unwrap())
    } else {
        req
    };

    // Headers
    let req = event.headers.into_iter().fold(req, |req, (k, v)| {
        req.insert_header((k.unwrap(), v.to_str().unwrap()))
    });

    // Body
    let req = req.set_payload(event.body.unwrap());

    Ok(req.to_request())
}
// impl TryFrom<LambdaRequest> for actix_http::Request {
//     type Error = LambdaError;

//     /// Actix-web Request from API Gateway event
//     fn try_from(event: LambdaRequest) -> Result<Self, Self::Error> {
